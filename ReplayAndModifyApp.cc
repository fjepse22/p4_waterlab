int main(int argc, char *argv[]) { return 0; }#include "ReplayAndModifyApp.h"
#include "ns3/log.h"
...
NS_LOG_COMPONENT_DEFINE ("ReplayAndModifyApp");
NS_OBJECT_ENSURE_REGISTERED (ReplayAndModifyApp);

TypeId
ReplayAndModifyApp::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::ReplayAndModifyApp")
    .SetParent<Application> ()
    .SetGroupName ("Tutorial")
    .AddConstructor<ReplayAndModifyApp> ();
  return tid;
}

ReplayAndModifyApp::ReplayAndModifyApp () {}
ReplayAndModifyApp::~ReplayAndModifyApp () {}

void
ReplayAndModifyApp::StartApplication ()
{
  m_socket = Socket::CreateSocket (GetNode (), TcpSocketFactory::GetTypeId ());
  m_local = InetSocketAddress (Ipv4Address::GetAny (), 9999);
  m_socket->Bind (m_local);
  m_socket->Listen ();
  m_socket->SetRecvCallback (MakeCallback (&ReplayAndModifyApp::HandleRead, this));
}

void
ReplayAndModifyApp::StopApplication ()
{
  if (m_socket)
    {
      m_socket->Close ();
...
void
ReplayAndModifyApp::HandleRead (Ptr<Socket> socket)
{
  Address from;
